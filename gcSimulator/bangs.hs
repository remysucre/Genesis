...
data Machine = Machine {
			  nursery :: !S.IntSet
			 , nursery_size :: !Int --Current size, in bytes, of the nursery
			 , nursery_limit :: !Int --How big can teh nursery get before a collection?
			 , heap_size :: !Int
			 , heap_limit :: !Int
			 , heap :: !G.Graph
			 , roots :: !S.IntSet --These contain _ONLY_ the static variable roots
			 , stacks:: !StackMap
			 , alloc :: !Int --How much total allocation?
		       }
		       deriving (Eq, Show)
...
{-Data type for GC stats -}
initMachine::Machine
initMachine = Machine S.empty 0 (4*mb) 0 (100*mb) G.empty S.empty M.empty 0
...
getRoots m = M.fold (S.union) S.empty (M.map stackRoots (stacks m) )
		...
		stackRoots s = F.foldl (S.union) (S.empty)  (fmap ( \(method,roots)->roots) s)
...
defaults::Options
defaults = Options{optNurseryLimit=1024*1024*4, optFileName="elephantTracks.trace"}

--Basically a fold...
flagsToOptions::[Flag]->Options->Options
flagsToOptions [] o = o
flagsToOptions ( (NurserySize ns):fs) o = flagsToOptions fs o{optNurseryLimit= read ns} 
flagsToOptions ( (FileName name):fs ) o  =  flagsToOptions fs o{optFileName = name}


options::[OptDescr Flag]
options = [ Option ['n'] ["nurserysize"]  (ReqArg NurserySize "" ) "Specify the nursery size (in bytes)"
           ,Option ['f'] ["file"] (ReqArg FileName "") "Specify the trace file name"
          ]
