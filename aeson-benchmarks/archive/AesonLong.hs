...
jstring_
  ...
  where isEscaped (((!(!((!((!(S (!(((!_)))) (!escaped))))))))))
        ...
        go (!(S a b)) (!c)
          ...
          | c == 34 = Nothing
          | otherwise =
            let (!(!(!a'))) = c == backslash in Just (S a' (a' || b))
          where (!backslash) = 92
           ...
           \ ptr ->
             do ((!(ret))) <- Z.parseT (go ptr) s
                case ret of
                    ...
                    (!((!((!(((!(Right (!p)))))))))) -> do let newlen
                                                                 = p `minusPtr` ptr
                                                               (!slop) = len - newlen
                                                           Right <$>
                                                             if slop >= 128 && slop >= len `quot` 4
                                                               then
                                                               B.create newlen $
                                                                 \ (!np) -> B.memcpy np ptr newlen
                                 ...
                                 if a < 55296 || a > 57343 then
                                   ...
                                   do b <- Z.string "\\u" *> hexQuad
                                      if a <= 56319 && b >= 56320 && b <= 57343 then
                                        let ((!(!c)))
                                              = ((a - 55296) `shiftL` 10) + (b - 56320) + 65536
